<template>
	<view style="padding-bottom: 20px;">
		...
		<view v-for="(item,index) in makeMeetList" :key="index" style="margin: 20px 15px 5px 15px;box-shadow: rgb(0 0 0 / 8%) 0px 0px 3px 2px;border-radius: 5px;" index="index">
			...
			<view class="metting-list-div" style="padding: 10px 20px 10px 20px;">
				<uni-row class="demo-uni-row">
					<uni-col :span="12">
						<view class="demo-uni-col" :style="{'font-size': '15px','font-weight': 900,color: auditStatusColor(item.auditStatus)}">
							{{ item.auditStatus }}
						</view>
					</uni-col>
					<uni-col :span="12">
						<view class="demo-uni-col" style="text-align: right;">
							<!-- 开会已结束还没上传照片 -->
							<nut-button size="mini" type="primary" icon="photograph" block @click="upload(item.id)" v-if="handleBtnShow(item)">
								上传图片
							</nut-button>
							
							<!-- 开会已结束并上传结束后的会议室照片 -->
							<nut-button size="mini" type="info" icon="image" block @click="showFn(item.id)" v-if="item.isUploadImg == '1'">
								查看图片
							</nut-button>

							<!-- 倒计时组件 -->
							<CountDown :key="index" :countDownTime="item.endTime" v-if="item.onState != '已结束' && item.auditStatus != '审核不通过'" />
							
							<!-- 审核不通过弹框 -->
							<view v-if="item.auditStatus == '审核不通过'">
								<nut-button type="default" size="mini" block @click="componentClick">原因</nut-button>
								<nut-dialog :noCancelBtn="nut_dialog" :noOkBtn="nut_dialog" :closeOnClickOverlay="nut_dialog" title="未通过原因"
									:content="item.auditRefuse" v-model:visible="visible">
								</nut-dialog>
							</view>
...
<script setup>
	...
	function handleBtnShow (item) {
		if (item.isUploadImg == '2' && item.onState == '已结束' && item.auditStatus == "审核通过") {
			return true;
		}
	}
	//不同的审核状态展示不同的颜色
	...
	const showFn = (makeMeetId) => {
		...
		uni.request({
			...
			success: (res) => {
				uni.hideLoading();
				if (res.data.Code != 1) {
					return uni.showToast({"title" : "图片获取失败", "icon" : "error"});
				}
				if (res.data.Data == null) {
					return uni.showToast({"title" : "暂无图片", "icon" : "success"});
				}
			...
			success: (chooseImageRes) => {
				...
				setTimeout(function () {
					...
					if (tempFilePaths.length == arr.length) {
						...
						for (let [index, elem] of makeMeetList.value.entries()) {
							if (elem.id == makeMeetId) {
								makeMeetList.value[index].isUploadImg = 1;
							}
						}
					}
				},5000);
